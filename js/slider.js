...
const slider = {
    ...
    render() {
        ...
        for(let i = 0; i < 2; i ++) {slider.appendChild(this.generateElement('div','slider-item',''))}
        
        // Здесь уже карточки делаем
        document.querySelectorAll('.slider-item').forEach((element, index) => element.appendChild(this.generateElement('div','item-about',`<p>${index + 1}</p>`)))
        ...
        /* Это кнопки слайдера, они ещё не доделаны. 5 утра всё таки, блин
